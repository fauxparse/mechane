...
- **Amended (issue #130).** Auto-layout is a **per-Frame mode, off by default**: a Frame either lays its children out (the depth above) or positions them absolutely. A director laying out a projector cue thinks in positions, and auto-layout-by-default is hostile to a new user. Note what this does *not* change — absolute-position overrides **within an auto-layout frame** stay deferred, exactly as written above. The deferral was always about one child opting out of its flex parent, never about whether a Frame must be a flex parent at all.
