...
import { ensureSceneCanvas, readCanvas } from "../db/canvas";
...
export const schema = createSchema<GraphQLContext>({
  typeDefs: /* GraphQL */ `
    ...
    type Query {
      ...
      "The Canvas owned by a Scene node."
  ...
  resolvers: {
    ...
    Query: {
      ...
      sceneCanvas: async (
        ...
        await findOwnShowOrThrow(showId, userId);
        const graphState = validGraphState(state ?? "draft");
        return serializeCanvas(await ensureSceneCanvas(showId, graphState, sceneNodeId));
      },
      blockCanvas: async (
        _parent,
