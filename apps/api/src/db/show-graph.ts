...
import {
  readCanvasById,
  replaceSceneCanvases,
  snapshotSceneCanvases,
  writeCanvasRows,
} from "./canvas";
...
async function writeGraph(
  ...
  const [current] = await tx
    ...
    .where(and(eq(showGraphs.showId, showId), eq(showGraphs.state, state)))
    .for("update");
  const previousSceneCanvases = current ? await snapshotSceneCanvases(current.id, tx) : [];
...
> {
  const result = await db.transaction(async (tx) => {
    ...
    const [draftGraph] = await tx
      .select({ id: showGraphs.id })
      .from(showGraphs)
      .where(and(eq(showGraphs.showId, showId), eq(showGraphs.state, "draft")));
    const draftSceneCanvases = draftGraph ? await snapshotSceneCanvases(draftGraph.id, tx) : [];
    ...
    const [publishedGraph] = await tx
      .select({ id: showGraphs.id })
      .from(showGraphs)
      .where(and(eq(showGraphs.showId, showId), eq(showGraphs.state, "published")));
    if (!publishedGraph) throw new Error(`Published graph for Show "${showId}" disappeared.`);
    await replaceSceneCanvases(
      tx,
      publishedGraph.id,
      draft.nodes.filter((node) => node.kind === "scene").map((node) => node.id),
      draftSceneCanvases,
      new Date(),
      true,
    );
