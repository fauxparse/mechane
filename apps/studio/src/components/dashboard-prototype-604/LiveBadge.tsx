...
//
// "Some kind of indicator if a show is live". Two sizes, because the variants
// need it at two densities: the full badge with the Run's elapsed time for a
// band or a header, and the bare dot for a card corner or a list row where
...
export function LiveDot({ className }: { className?: string }) {
  return (
    <span
      role="img"
      aria-label="Live"
      className={cn("size-2 shrink-0 animate-pulse rounded-full bg-live", className)}
    />
  );
}
