...
//
// A Scene painted small. `CanvasRenderer` paints at the authored Artboard size
// in real DOM, so a thumbnail is that output under a `scale()` — not a separate
// small-size renderer. The container is measured rather than assumed because
// every variant here gives previews a different width.
...
export interface ShowPreviewProps {
   ...
   * a fixed height (`h-full` inside an `h-[42vh]` parent) to get a box whose
   * width follows the Scene's aspect ratio, so there is no letterboxing.
   */
  shape?: "box" | "scene";
}
...
export function ShowPreview({
  ...
  return (
    <div
      ref={boxRef}
      // The preview is a picture, not a control: the card underneath owns the
