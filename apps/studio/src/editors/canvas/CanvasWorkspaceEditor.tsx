...
/** The Canvas editor owns interaction state, gesture lifecycles, and commit rules. */
export function CanvasWorkspaceEditor({
  ...
  const finishCreation = (event: PointerEvent<HTMLElement>, cancel = false) => {
    ...
    const artId = canvasForCreation(canvasTargets, rect, {
      x: event.clientX,
      y: event.clientY,
    });
  ...
  const resizePreview =
    ...
    selectedGeometry?.rootElementId === resizeDraft.subjects[0]?.elementId
      ...
      : null;
  const resizeCursor = resizeDraft ? handleCursor(resizeDraft.handle) : null;
