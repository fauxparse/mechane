...
export function ShapeDefaultEditor({
  field,
  shapes,
  onChange,
  onOpenShape,
    ...
    if (!shape)
      return (
        <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
          The referenced Shape is unavailable.
        </div>
      );
    const objectValue =
      ...
      <section className="rounded-lg border border-border p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{shape.name}</p>
          </div>
          <button
            type="button"
            className="text-xs font-medium text-primary hover:underline"
            onClick={() => onOpenShape(shape.id)}
          >
            Open Shape
          </button>
        </div>
        <div className="mt-4 grid gap-4">
          {shape.fields.map((field) => (
            <ValueEditor
              key={field.id}
              type={field.type}
              value={objectValue[field.id] ?? defaultValueForType(field.type, shapes)}
              shapes={shapes}
              label={field.name}
  ...
  return (
    <section className="rounded-lg border border-border p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1.5 text-xs font-medium hover:bg-muted"
          onClick={() => onChange([...items, defaultValueForType(type, shapes)])}
        >
        ...
        {items.map((item, index) => (
          <div key={index} className="rounded-md border border-border bg-muted/20 p-3">
            ...
            <ValueEditor
              ...
              onChange={(next) =>
                onChange(
                  items.map((candidate, itemIndex) => (itemIndex === index ? next : candidate)),
                )
              }
              onOpenShape={onOpenShape}
            />
  ...
  return (
    <label className="grid gap-2 text-sm font-medium">
      ...
      <input
        ...
        className="h-9 rounded-md border border-input bg-background px-3 font-normal outline-none focus-visible:ring-2 focus-visible:ring-ring"
        onChange={(event) => {
          if (type !== "number") {
