...
import { coalesceGraphEdits } from "./graph-edits";
...
/** Reduces repeated absolute Canvas writes while preserving edit order. */
export function coalesceCanvasEdits(edits: readonly CanvasEdit[]): CanvasEdit[] {
  const superseded = new Set<number>();
  const seen = new Set<string>();
  for (let index = edits.length - 1; index >= 0; index -= 1) {
    const edit = edits[index] as CanvasEdit;
    const structuralId =
      edit.type === CANVAS_COMMAND_TYPES.addElement
        ? edit.element.id
        : edit.type === CANVAS_COMMAND_TYPES.removeElement
          ? edit.elementId
          : null;
    if (structuralId) {
      for (const key of seen) {
        if (key.startsWith(`${structuralId}:`)) seen.delete(key);
      }
    }
    const keys =
      edit.type === CANVAS_COMMAND_TYPES.updateElement && Object.keys(edit.properties).length === 1
        ? Object.keys(edit.properties).map((key) => `${edit.elementId}:${key}`)
        : edit.type === CANVAS_COMMAND_TYPES.reparentElement
          ? [`${edit.elementId}:parent`]
          : [];
    if (keys.some((key) => seen.has(key))) superseded.add(index);
    for (const key of keys) seen.add(key);
  }
  return superseded.size === 0 ? [...edits] : edits.filter((_, index) => !superseded.has(index));
}

/** Coalesces a mixed graph/Canvas batch without moving either edit stream. */
export function coalesceShowEdits(edits: readonly ShowEdit[]): ShowEdit[] {
  const graph = edits.filter((edit): edit is GraphEdit => edit.type.startsWith("graph."));
  const canvas = edits.filter((edit): edit is CanvasEdit => edit.type.startsWith("canvas."));
  const keptGraph = new Set(coalesceGraphEdits(graph));
  const keptCanvas = new Set(coalesceCanvasEdits(canvas));
  return edits.filter((edit) =>
    edit.type.startsWith("graph.")
      ? keptGraph.has(edit as GraphEdit)
      : keptCanvas.has(edit as CanvasEdit),
  );
}
