...
describe("assertValidShowGraph", () => {
  it("accepts the empty graph — a Show with no Flows is valid", () => {
    expect(assertValidShowGraph(emptyShowGraph())).toEqual({
      shapes: [],
      nodes: [],
      edges: [],
      cues: [],
...
describe("edge layout keys", () => {
  ...
  it("reads back what it writes", () => {
    for (const key of [edgeLayoutKey(2), edgeLayoutKey(0, "head"), edgeLayoutKey(4, "tail")]) {
      expect(parseEdgeLayoutKey(key)).not.toBeNull();
    }
    expect(parseEdgeLayoutKey("2")).toEqual({ runIndex: 2, jog: null });
    expect(parseEdgeLayoutKey("0.head")).toEqual({ runIndex: 0, jog: "head" });
    expect(parseEdgeLayoutKey("4.tail")).toEqual({ runIndex: 4, jog: "tail" });
  });
