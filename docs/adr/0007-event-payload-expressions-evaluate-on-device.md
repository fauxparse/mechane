...
*Originally titled "Event payload expressions evaluate on the Device, at fire time" and scoped to payloads alone; widened by #140 when property expressions turned out to need the same treatment for the same reasons. The line that once read "the exception is narrow: it covers Event payloads only" is replaced by the scope paragraph below.*
