...
This ADR applies to server-visible Player state changes. A per-connection Navigate changes only the browser-local Player aggregate, so its anonymous Event ledger write creates no invalidation outbox row or realtime message. A shared Show-state mutation increments the Show `stateSequence` and coalesces the pending outbox row to the greatest sequence in the same transaction. The realtime payload is `{ stateSequence }`, not a payload-free marker. Players refetch the snapshot and apply it only when its sequence is at least the sequence acknowledged by their optimistic result; older snapshots cannot discard a newer local overlay. If a future Action changes shared Show state, that mutation, sequence increment, and invalidation remain one transaction.
