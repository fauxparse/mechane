...
- To keep the real flow exercised in dev/tests, seed one _unverified_ user as well (or rely on manual sign-up) and verify through the captured Mailpit message — Mailpit's [REST API](https://mailpit.axllent.org/docs/api-v1/) can fetch the message and extract the URL programmatically for integration tests.
